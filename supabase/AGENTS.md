...
- The vault schema is deliberately locked to the direct backend Postgres path across TWO ordered migrations: the enablement migration (`enable_supabase_vault`) enables the extension and revokes PUBLIC/anon/authenticated access (schema, `vault.secrets`, `vault.decrypted_secrets`, and the secret-management functions), and the follow-up `lock_vault_from_service_role` migration enforces the service_role lockout. The split is required because Supabase's platform post-create handling re-establishes the platform's service_role Vault grants after the extension-enabling migration reports successful (observed on hosted previews) — same-migration service_role revokes are silently re-granted away, so the lockout must run in a subsequent migration after that handling has settled. The migrating role needs no grants. This is a Vault-specific least-privilege restriction: OpenOrc still uses the privileged Supabase service-role/secret credential for other Supabase-owned capabilities (Auth, administrative lifecycle), and nothing here disables, removes, or broadly reduces service_role outside `vault`.
- The effective privilege posture is proven against a real Supabase branch by integration assertions (`has_schema_privilege`/`has_table_privilege`/`has_function_privilege` in `tests/integration/test_connection_credential_services.py`), not inferred from migration text; preview-deployment success alone does not prove the posture.
