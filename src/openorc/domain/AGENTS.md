...
- Each ReviewIteration references the exact subject reviewed: v1 planning iterations bind a specific same-Task PlanRevision (durably enforced by composite foreign keys); PR review support later binds one TaskPullRequest plus exact head SHA on the same review-history model — no second model. Iteration numbering is unique per ReviewLoop.
