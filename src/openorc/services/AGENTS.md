...
## Administrative deletion and account lifecycle (issue #97)

These conventions are established by `services/administrative_deletion.py`, `services/account_lifecycle.py`, and `services/profile_lifecycle_guard.py`; later Phase 2 leaves inherit them. Changing one is a deliberate architecture decision, not an implementation detail.

- The destructive administrative use cases (disconnect an Agent Runtime Connection with its secure-secret cleanup, hard-delete an unreferenced Connection, purge an archived Task attempt, delete a Repository mapping / Project / Workspace) compose the same fail-closed sequence: the account-wide Owner-mutation barrier first, then the #53 Workspace ownership gate, then the destructive effect. Missing and cross-Workspace subjects are uniformly `NotFoundError` (the anti-probing rule). Deleting OpenOrc state never deletes, closes, reverts, or mutates GitHub or runtime-owned artifacts; these modules import no adapter, which is the structural guarantee that ordinary aggregate deletion makes zero external calls.
- Credential cleanup removes ONLY the OpenOrc-owned Vault secret referenced by the opaque `auth_reference`, inside the same short `composed_transaction` as the row mutation, with every affected Connection row-locked before its current reference is inspected. Malformed and dangling references fail closed (`ConflictError`) — cleanup never reports success when the persisted reference cannot be reconciled — and the surrounding transaction rolls back so no partial cleanup commits. The Phase 1 restrictive delete primitive's `ForeignKeyViolation` is translated into the typed conflict (history is never cascaded away to make a delete succeed); only archived Tasks may be purged (`TaskDomainError` → `ConflictError`).
- The permanent account deletion is the one operation crossing the Supabase Auth network boundary: revoke every OpenOrc-owned Vault secret and disconnect every Connection (and claim the durable attempt state) in ONE short transaction, commit, then call the Auth Admin boundary with NO database transaction open, and rely on the sanctioned `auth.users → profiles` cascade — never a competing manual graph deletion, never a session blacklist, never Supabase soft deletion.
- The durable attempt state machine (`openorc.profiles` attempt columns, composite-CHECK-bound) drives recovery: normal → `active` claim composed with the revocation; fresh `active` is durable single-flight (`ConflictError`, no external call); lease-EXPIRED `active` and `uncertain` states reconcile through the Admin read surface BEFORE any replay; every reconciliation result is bound to the EXACT attempt UUID it reconciled and the replay claim is a compare-and-swap on that exact identity — a stale reconciliation matches zero rows and the caller reloads and reclassifies instead of replaying. The active-attempt lease is derived from the Admin client's bounded request timeouts plus a documented safety margin and is compared against the DATABASE clock. Known failures clear the attempt attempt-scoped (account present, credentials revoked, explicit retry safe); unreconciled uncertainty keeps the state fail-closed; success cascade-removes the state with the Profile.
- Profile absence at entry (an already-deleted account) routes to read-surface reconciliation only — confirmed absent is the idempotent `already_absent` end state, confirmed present is an inconsistent durable state that fails closed — never a blind destructive call and never a manufactured Profile.
- The account-wide Owner-mutation barrier (`require_account_operational`, the `FOR KEY SHARE` Profile read) is composed as the FIRST lock acquisition of every authenticated Owner mutation that carries a `profile_id`: Workspace configuration (#53), Connection credential administration (#55 — whose configure additionally rejects disabled Connections so `disconnect` is the revocation barrier), and the #97 administrative deletion operations. `delete_account` composes no guard (it must stay retryable); authentication and ordinary reads are unaffected. The #54 Task mutation primitives remain authority-neutral and are deliberately NOT guarded — later Owner-facing workflow capabilities compose the barrier at their Owner authorization boundary; changing #54 is a separate deliberate architecture decision.
- `SUPABASE_SECRET_KEY` stays optional on the shared Settings surface: authentication ownership follows direct consumption, and the requirement is enforced where the Auth Admin client is constructed, not forced into every process that boots from `Settings.from_env`.
