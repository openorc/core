...
## Auth Admin boundary (issue #97)

These conventions are established by `adapters/supabase/admin.py`; later Phase 2 leaves inherit them. Changing one is a deliberate architecture decision, not an implementation detail.
...
- The deployment's secret API key is NOT a JWT (verified against the current Supabase API-keys documentation): it travels on the `apikey` request header only — never as `Authorization: Bearer`, never in a URL or query parameter, and on no other header.
