...
- Canonical-branch ownership is Task-level and current-only: `UNIQUE (repository_id, canonical_feature_branch) WHERE archived_at IS NULL AND canonical_feature_branch IS NOT NULL` makes a non-null branch exclusive among a Repository's current Tasks; archived attempts release it. `bind_canonical_branch` rotates the token like every other mutation. Executions (#24) may reference/observe repository state but never own the canonical branch.
